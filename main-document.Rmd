...
differs from their gender identity [@krieger2003]. Gender identity is
distinct from sex and sexual orientation and consists of how one
identifies and conceptualizes onself as a gendered being along a
spectrum of male, female. Some individuals identify as both or neither
male or female because they are not considered inclusive enough to
comprise the range gender experiences. Gender identity refers to felt
sense of self (Tate). It is often perceived outwardly through observable
presentation that reflects internal identity, such as through behavior
or appearance. Alternately, sexual orientation describes external
physical, emotional, and sexual attraction toward others.
[@krieger2003]. It is important to note that sexual orientation is
independent of sex and gender and not synonymous but nevertheless
linked. Life experiences and how one is perceived socially is influenced
by the presence of secondary sex characteristics. This in mind,
questions regarding sex and gender are implicitly receiving both
biological and social information, or secondary sex characteristics and
gender sentiment [@johnson2009].

Both social science and medical research request sex and gender
information to include in analysis. However, definitions have been found
to vary across disciplines [@bauer2017]. Queries to ascertain dimensions
of sex and gender depend on the intended outcome specific to the
research question [@bauer2017]. For example, surveys might erroneously
utilize "gender" when explaining biological differences rather than sex
[@johnson2009]. Sex and gender measures have developed from early
conventions of assumption recording in telephone and in-person surveys
to single item questions, neither of which are capable of fully
assessing sex and gender [@bauer2017\]. Today, most methods of recording
sex and gender in large scale medical and public health surveys feature
some variation of a two-item question, check-all-that-apply, or write-in
response. Although these methods are more robust in assessing sex and
gender among participants, there are still numerous elements of sex that
are more difficult to determine from queries such as physiology, genes,
and hormones. This is also the case with elements of gender, like
ascribed and perceived gender roles \[@johnson2009].
...
BRFSS_all_clean %>% 
  ...
  group_by(TRNSGNDR, SEX, yearLong) %>% 
  count() %>% 
  group_by(SEX, yearLong) %>% 
  mutate(percYear = 100 * n / sum(n)) %>% 
  ggplot(aes(x = SEX, fill = TRNSGNDR, y = percYear)) +
  geom_col(position = "dodge") +
  #coord_flip() +
  scale_x_discrete(labels = function(x) stringr::str_wrap(x, width = 10)) +
  facet_wrap(~yearLong, 
  ...
  theme_minimal() +
  labs(x = "Sex category", y = "Percent in each sex category",
       fill = "Gender identity")  

```

```{r}
#line chart with sex and gender over time
BRFSS_all_clean %>% 
  drop_na() %>% 
  ...
  group_by(SEX, year) %>% 
  mutate(percYear = 100 * n / sum(n)) %>% 
  ggplot(aes(x=year, y=percYear))+
  geom_point(aes(shape=factor(SEX), color= factor(TRNSGNDR)))+
  geom_line(aes(linetype=SEX, color=TRNSGNDR))+
    labs(x="Year",
       y="Percent Respondants",title="BRFSS Sex and Transgender Responses, 2015-2020")
```


+----------+-----------------------------+----------+----------+
| Q u e s  | Question                    | A s k e  | Y ea r(  |
| t i o n  |                             | d o f .  | s)       |
| i d      |                             | . .      |          |
+==========+=============================+==========+==========+
| \        |                             | c o m p  | 20 15    |
...
| K W K 1  |                             |          |          |
+----------+-----------------------------+----------+----------+
| \        | Binge drinkers (males       | c o m p  | 20 15 ,  |
| \_ R F B | having five or more drinks  | u t e d  | 20 18    |
| I N G 5  | on one occasion, females    |          |          |
|          | having four or more drinks  |          |          |
|          | on one occasion)            |          |          |
+----------+-----------------------------+----------+----------+
| \        | Heavy drinkers (adult men   | c o m p  | 2 01 5,  |
| \_ R F D | having more than 14 drinks  | u t e d  | 2 01 6,  |
| R H V 5  | per week and adult women    |          | 20 17    |
|          | having more than 7 drinks   |          |          |
|          | per week)                   |          |          |
+----------+-----------------------------+----------+----------+
| \        | Heavy drinkers (adult men   | c o m p  | 20 18    |
| \_ R F D | having more than 14 drinks  | u t e d  |          |
| R H V 6  | per week and adult women    |          |          |
|          | having more than 7 drinks   |          |          |
|          | per week)                   |          |          |
+----------+-----------------------------+----------+----------+
